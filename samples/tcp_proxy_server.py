...
from bacpypes.bsllservice import TCPServerMultiplexer, ProxyServerService, UserInformation
...
test_users = {test_user.username: test_user}
test_static_users = {}

class TestProxyServer(ProxyServerService, Logging):

    _authenticationRequired = True

    def get_default_user_info(self, addr):
        if _debug: TestProxyServer._debug("get_default_user_info %r", addr)

        return test_static_users.get(addr, None)

    def get_user_info(self, username):
        if _debug: TestProxyServer._debug("get_user_info %r", username)

        return test_users.get(username, None)

#
#   TestProxyApplication
#

class TestProxyApplication(Application, Logging):

    def __init__(self, localDevice, localAddress, aseID=None):
        if _debug: TestProxyApplication._debug("__init__ %r %r aseID=%r", localDevice, localAddress, aseID)
...
class TestConsoleCmd(ConsoleCmd, Logging):
    ...
    def do_nsap(self, args):
        """nsap"""
        args = args.split()
        TestConsoleCmd._debug("do_nsap %r", args)

        this_application.nsap.debug_contents()
...
try:
    ...
    this_application = TestProxyApplication(this_device, addr)
